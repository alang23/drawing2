...
{
    var defaults = { // 榛樿鍊笺€� 
        title:'',       // 鏍囬鏂囨湰锛岃嫢涓嶆兂鏄剧ずtitle璇烽€氳繃CSS璁剧疆鍏禿isplay涓簄one
        showTitle:true,     // 鏄惁鏄剧ず鏍囬鏍忋€�
        closeText:'', // 鍏抽棴鎸夐挳鏂囧瓧锛岃嫢涓嶆兂鏄剧ず鍏抽棴鎸夐挳璇烽€氳繃CSS璁剧疆鍏禿isplay涓簄one
    ...
    var dialog = $('<div id="' + options.id + '" class="dialog" onclick="closediv();">'+barHtml+'<div class="content"></div></div>').hide();
    $('body').append(dialog);
            ...
            switch(c.type.toLowerCase())
            {
                ...
                case 'url':
                    ...
                    $.ajax({url:c.value,
                        ...
                        error:function(xml,textStatus,error){div.html('正在加载....')}
                    });
                    break;
                case 'iframe':
