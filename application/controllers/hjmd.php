...
class Hjmd extends Base_Controller
{
	...
	public function index()
	{
		...
		$daybest = array();
		$daybestwhere['page'] = true;
        $daybestwhere['limit'] = 6;
        $daybestwhere['offset'] = 0;
		$daybestwhere['order'] = array('key'=>'`w.ji`','value'=>'DESC'); 
		$daybest = $this->works->getList($daybestwhere);
		$data['daybest'] = $daybest;
	...
	public function choujiang()
	{
		...
		// print_r($user);
		 ...
		 if(($user['piao']) < 3){
		 	$json = array('err'=>-2,'msg'=>'没有抽奖机会了！点赞三次即可抽奖，快去点赞吧！');
		 	echo json_encode($json);
		 	exit;
		 }
		...
		public function choujiang2()
	{
		...
		// print_r($user);
		 ...
		 $checklotterylog = $this->lottery_logs->get_new_by_id($checkconfig);
		 if(!empty($checklotterylog)){
		 	$json = array('err'=>2,'msg'=>'很遗憾，没中奖','biao'=>100);
		 	echo json_encode($json);
		 	exit;
		 }
		 ...
		 */
		 ...
		 if(empty($lottery)){
			...
			if($rad == 1 || $rad == 2 || $rad == 4  || $rad ==7 || $rad == 6){
				$json = array('err'=>2,'msg'=>'很遗憾，没中奖','biao'=>100);
				echo json_encode($json);
				exit;
			}
